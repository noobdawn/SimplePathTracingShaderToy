...
#iChannel0 "self"
#iChannel1 "file://路径追踪第四部分/env.png"
...
bool TestHitScene(ray r, inout hitInfo hit) {
    ...
    const vec3 wallSpecularColor = vec3(0.0f);
    const float wallRoughness = 1.0f;
    // 测试球体
    if (TestHitSphere(r, vec3(-3.0f, 5.0f, 0.0f), 0.5f, hit)) {
        hit.albedo = wallAlbedo;
        hit.emission = opaqueEmission;
        hit.specularColor = vec3(0.3f, 1.0f, 0.3f);
        hit.roughness = 0.0f;
    }
    if (TestHitSphere(r, vec3(-1.5f, 5.0f, 0.0f), 0.5f, hit)) {
        hit.albedo = wallAlbedo;
        hit.emission = opaqueEmission;
        hit.specularColor = vec3(0.3f, 1.0f, 0.3f);
        hit.roughness = 0.25f;
    }
    if (TestHitSphere(r, vec3(0.0f, 5.0f, 0.0f), 0.5f, hit)) {
        hit.albedo = wallAlbedo;
        hit.emission = opaqueEmission;
        hit.specularColor = vec3(0.3f, 1.0f, 0.3f);
        hit.roughness = 0.5f;
    ...
    if (TestHitSphere(r, vec3(0.0f, 1.0f, 0.0f), 2.0f, hit)) {
        hit.albedo = wallAlbedo;
        hit.emission = opaqueEmission;
        hit.specularColor = vec3(1.0f, 0.3f, 1.0f);
        hit.roughness = 0.0f;
    }
    // 测试盒子
    ...
    if (TestHitQuad(r, 
        ...
        hit)) {
            hit.albedo = wallAlbedo;
            hit.emission = opaqueEmission;
            hit.specularColor = wallSpecularColor;
            hit.roughness = wallRoughness;
    }
    // 朝相机的一面不做测试
    // 左侧面
    if (TestHitQuad(r, 
